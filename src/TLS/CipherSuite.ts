...
export type KeyExchangeAlgorithm =
  ...
  | "psk"
  | "dhe_psk"
  | "rsa_psk"; // Server/Client|KeyExchange: see https://tools.ietf.org/html/rfc4279#page-4
 ...
 */
export function createMAC(algorithm: HashAlgorithm): GenericMacDelegate {
  ...
  const ret = ((
    ...
    const mac_key =
      sourceConnEnd === "server"
        ? keyMaterial.server_write_MAC_key
        : keyMaterial.client_write_MAC_key;
    // and return the hash
...
export class CipherSuite extends TLSStruct {
  ...
  constructor(
    ...
    public readonly algorithm?:
      ...
      | AEADCipher.AEADCipherAlgorithm,
  ...
  private createCipher(): GenericCipherDelegate {
    const ret = (() => {
      switch (this.cipherType) {
        ...
        default:
          throw new Error(
            `createCipher not implemented for ${this.cipherType} cipher`
          );
      }
    })();
  ...
  public specifyCipher(
    keyMaterial: KeyMaterial,
    connEnd: ConnectionEnd
  ): CipherDelegate {
    const ret = ((plaintext: DTLSCompressed) =>
      this.Cipher(plaintext, keyMaterial, connEnd)) as CipherDelegate;
  ...
  private createDecipher(): GenericDecipherDelegate {
    const ret = (() => {
      switch (this.cipherType) {
        ...
        default:
          throw new Error(
            `createDecipher not implemented for ${this.cipherType} cipher`
          );
      }
    })();
    if (!ret.keyLength) ret.keyLength = 0;
  ...
  public specifyDecipher(
    keyMaterial: KeyMaterial,
    connEnd: ConnectionEnd
  ): DecipherDelegate {
    const ret = ((packet: DTLSCiphertext) =>
      this.Decipher(packet, keyMaterial, connEnd)) as DecipherDelegate;
  ...
  private createMAC(): GenericMacDelegate {
    switch (this.cipherType) {
      ...
      default:
        throw new Error(
          `createMAC not implemented for ${this.cipherType} cipher`
        );
    }
  }
}
