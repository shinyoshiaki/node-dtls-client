...
export class ConnectionState {
  ...
  public get Cipher(): CipherDelegate {
    if (this._cipher == undefined) {
      this._cipher = this.cipherSuite.specifyCipher(
        this.key_material,
        this.entity
      );
    }
  ...
  public get Decipher(): DecipherDelegate {
    if (this._decipher == undefined) {
      this._decipher = this.cipherSuite.specifyDecipher(
        this.key_material,
        this.entity
      );
    }
  ...
  private computeKeyMaterial(): void {
    const keyBlock = PRF[this.cipherSuite.prfAlgorithm](
      ...
      2 *
        (this.cipherSuite.MAC.keyAndHashLength +
          this.cipherSuite.Cipher.keyLength +
          this.cipherSuite.Cipher.fixedIvLength)
    );
