...
export class TLSStruct {
    ...
    for (const def of this.propertyDefinitions) {
      ...
      switch (type.type) {
        ...
        case "enum":
          ...
          result = {
            result: bufferToNumber(buf, bitSize, offset + delta),
            readBytes: bitSize / 8,
          };
          break;
        case "vector":
        ...
        case "buffer":
          if (type.maxLength === Number.POSITIVE_INFINITY) {
            ...
            if (TypeSpecs.Buffer.isVariableLength(type)) {
              const lengthBits = (8 *
                util.fitToWholeBytes(type.maxLength)) as BitSizes;
            ...
            buf.copy(
              ret,
              0,
              offset + delta + lengthBytes,
              offset + delta + lengthBytes + length
            );
  ...
  public serialize(): Buffer {
    const ret = this.propertyDefinitions.map((def) => {
      // Welche Eigenschaft wird ausgelesen?
      const propName = def.name;
      const type = def.type;
