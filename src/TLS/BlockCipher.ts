...
export function createCipher(
  ...
  const ret = ((
    ...
    const MAC = mac(
      Buffer.concat([packet.computeMACHeader(), packet.fragment]),
      keyMaterial,
      connEnd
    );

    // combine that with the MAC to form the plaintext and encrypt it
    const plaintext = Buffer.concat([packet.fragment, MAC]);
    ...
    const fragment = Buffer.concat([record_iv, ciphertext]);
...
export function createDecipher(
  ...
  const ret = ((
    ...
    const expectedMAC = mac(
      Buffer.concat([result.computeMACHeader(), result.fragment]),
      keyMaterial,
      sourceConnEnd
    );
