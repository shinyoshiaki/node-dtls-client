...
export class Vector<T extends number | ISerializable> {
  ...
  public serialize(spec: TypeSpecs.Vector): Buffer {
    ...
    switch (spec.itemSpec.type) {
      ...
      case "enum":
        ...
        serializedItems = (this.items as number[]).map((v) =>
          numberToBuffer(v, bitSize)
        );
        break;

      case "struct":
        serializedItems = (this.items as ISerializable[]).map((v) =>
          v.serialize()
        );
    }
    ...
    if (TypeSpecs.Vector.isVariableLength(spec)) {
      ...
      ret = Buffer.concat([numberToBuffer(ret.length, lengthBits), ret]);
    }
    return ret;
  }
    ...
    switch (spec.itemSpec.type) {
      ...
      case "enum":
        ...
        for (i = 0; i < length; i += bitSize / 8) {
          this.items.push(
            (bufferToNumber(buf, bitSize, offset + delta) as any) as T
      ...
      case "struct":
        i = 0;
        while (i < length) {
          const item = spec.itemSpec.structType.from(
            spec.itemSpec,
            buf,
            offset + delta
          );
          if (item.readBytes <= 0) {
            ...
            throw new Error(
              `Zero or less bytes read while parsing TLS struct.`
            );
          }
          i += item.readBytes;
  ...
  public static from<T extends number | ISerializable>(
    spec: TypeSpecs.Vector,
    buf: Buffer,
    offset?: number
  ): DeserializationResult<Vector<T>> {
