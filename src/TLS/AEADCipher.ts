...
const AEADCipherParameters: {
  [algorithm in AEADCipherAlgorithm]?: AEADCipherParameter;
} = {
  "aes-128-ccm": {
    interface: ccm,
    keyLength: 16,
...
export function createCipher(
  ...
  const ret = ((
    ...
    const salt =
      connEnd === "server"
        ? keyMaterial.server_write_IV
        : keyMaterial.client_write_IV;
    const nonce_explicit = crypto.pseudoRandomBytes(
      cipherParams.recordIvLength
    );
    // alternatively:
    ...
    const cipher_key =
      connEnd === "server"
        ? keyMaterial.server_write_key
        : keyMaterial.client_write_key;
...
export function createDecipher(
  ...
  const ret = ((
    ...
    const sourceConnEnd: ConnectionEnd =
      connEnd === "client" ? "server" : "client";
    ...
    const additionalData = new AdditionalData(
      ...
      packet.fragment.length -
        decipherParams.recordIvLength -
        decipherParams.authTagLength
    ).serialize();
    ...
    const ciphered = ciphertext.slice(
      decipherParams.recordIvLength,
      -decipherParams.authTagLength
    );
