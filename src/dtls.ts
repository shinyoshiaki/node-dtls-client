...
export namespace dtls {
  ...
  export class Socket extends EventEmitter {
    ...
    public send(data: Buffer, callback?: SendCallback) {
      ...
      if (!this._handshakeFinished) {
        throw new Error(
          "DTLS handshake is not finished yet. Cannot send data."
        );
      }
    ...
    private udp_onListening() {
      ...
      this.handshakeHandler = new ClientHandshakeHandler(
        this.recordLayer,
        this.options,
      ...
      for (const msg of messages) {
        switch (msg.type) {
          case ContentType.handshake:
            const handshake = TLSStruct.from(FragmentedHandshake.spec, msg.data)
          ...
          case ContentType.alert:
            ...
            if (alert.level === AlertLevel.fatal) {
              ...
              const errorMessage = `received fatal alert: ${
  ...
  function checkOptions(opts: Options) {
    ...
    if (opts.type !== "udp4" && opts.type !== "udp6")
      throw new Error(
        `The connection options must have a "type" property with value "udp4" or "udp6"!`
      );
    if (typeof opts.address !== "string" || opts.address.length === 0)
      throw new Error(
        `The connection options must contain the remote address as a string!`
      );
    if (typeof opts.port !== "number" || opts.port < 1 || opts.port > 65535)
      throw new Error(
        `The connection options must contain a remote port from 1 to 65535!`
      );
    if (typeof opts.psk !== "object")
      throw new Error(
        `The connection options must contain a PSK dictionary object!`
      );
  }
