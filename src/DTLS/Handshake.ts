...
export class FragmentedHandshake extends TLSStruct {
  ...
  public isFragmented(): boolean {
    return (
      this.fragment_offset !== 0 || this.total_length > this.fragment.length
    );
  }
   ...
   */
  public static findAllFragments(
    fragments: FragmentedHandshake[],
    reference: FragmentedHandshake
  ): FragmentedHandshake[] {
    // ignore empty arrays
  ...
  public static isComplete(fragments: FragmentedHandshake[]): boolean {
    ...
    const ranges = fragments
      ...
      .map((f) => ({
        start: f.fragment_offset,
        end: f.fragment_offset + f.fragment.length - 1,
      }))
    ...
    const noHoles = ranges.every((val, i, arr) => {
      ...
      if (i === arr.length - 1) {
        if (val.end !== totalLength - 1) return false;
      }
      // no problems
      return true;
    });
  ...
  public split(maxFragmentLength?: number): FragmentedHandshake[] {
    ...
    if (maxFragmentLength == null) {
      maxFragmentLength =
        RecordLayer.MAX_PAYLOAD_SIZE - FragmentedHandshake.headerLength;
    }
...
export class ClientHello extends Handshake {
  public static readonly __spec = {
    ...
    cookie: Cookie.spec,
    cipher_suites: TypeSpecs.define.Vector(
      CipherSuite.__spec.id,
      2,
      2 ** 16 - 2
    ),
    compression_methods: TypeSpecs.define.Vector(
      CompressionMethod.spec,
      1,
      2 ** 8 - 1
    ),
...
export class HelloVerifyRequest extends Handshake {
  ...
  constructor(public server_version: ProtocolVersion, public cookie: Buffer) {
...
export class ServerKeyExchange_PSK extends TLSStruct {
  ...
  constructor(public psk_identity_hint: Buffer) {
...
export class ClientKeyExchange_PSK extends TLSStruct {
  ...
  constructor(public psk_identity: Buffer) {
